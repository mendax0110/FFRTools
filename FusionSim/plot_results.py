#!/usr/bin/env python3
"""
FusionSim Data Analysis and Visualization

Loads particle simulation data from CSV, computes derived physical
quantities, and produces static or interactive plots.
"""
...
CHARGE_TOLERANCE = 1e-20
UNKNOWN_COLOR = '#333333'

# Fusor grid radii, referenced by several plots.
INNER_GRID_RADIUS_CM = 1.6
OUTER_GRID_RADIUS_CM = 8.0


def get_particle_style(ptype: str) -> tuple[str, str]:
    """
    Look up display color and label for a particle type, with a sane
    fallback for anything not in PARTICLE_TYPES.
    :param ptype: The particle type key (e.g. 'deuterium', 'unknown').
    :return: (color, label) tuple.
    """
    props = PARTICLE_TYPES.get(ptype, {})
    color = props.get('color', UNKNOWN_COLOR)
    label = props.get('label', ptype.capitalize())
    return color, label


def classify_particle(mass: float, charge: float) -> str:
    """
    Classify a single particle based on mass and charge.
    Kept for callers that need a scalar classifier; load_data uses the
    vectorized version below for performance on large datasets.
    :param mass: The mass of the particle.
    :param charge: The charge of the particle.
    :return: The matching particle type key, or 'unknown'.
    """
    for ptype, props in PARTICLE_TYPES.items():
        mass_min, mass_max = props['mass_range']
        if mass_min <= mass <= mass_max:
            if abs(charge - props['charge']) < CHARGE_TOLERANCE:
                return ptype
    return 'unknown'


def classify_particle_vectorized(mass: np.ndarray, charge: np.ndarray) -> np.ndarray:
    """
    Vectorized equivalent of classify_particle, evaluated over whole
    columns instead of row-by-row. Same first-match-wins order as the
    scalar version, since PARTICLE_TYPES iteration order is preserved.
    :param mass: Array of particle masses.
    :param charge: Array of particle charges.
    :return: Array of particle type strings.
    """
    conditions = []
    choices = []
    for ptype, props in PARTICLE_TYPES.items():
        mass_min, mass_max = props['mass_range']
        cond = (mass >= mass_min) & (mass <= mass_max) & (np.abs(charge - props['charge']) < CHARGE_TOLERANCE)
...
def load_data(filepath) -> pd.DataFrame:
    ...
    df['particle_type'] = classify_particle_vectorized(df['mass'].to_numpy(), df['charge'].to_numpy())
...
def plot_comprehensive(df: pd.DataFrame):
    """
    ...
    for ptype in df['particle_type'].unique():
        mask = df['particle_type'] == ptype
        if mask.sum() > 0:
            color, label = get_particle_style(ptype)
    ...
    ax2.axvline(x=INNER_GRID_RADIUS_CM, color='red', linestyle='--', linewidth=1.5,
                label=f'Inner Grid ({INNER_GRID_RADIUS_CM} cm)')
    ax2.axvline(x=OUTER_GRID_RADIUS_CM, color='blue', linestyle='--', linewidth=1.5,
                label=f'Outer Grid ({OUTER_GRID_RADIUS_CM} cm)')
    ax2.legend(loc='upper right', fontsize=8)

    ax3 = fig.add_subplot(2, 3, 3, projection='polar')
    theta = np.arctan2(df['y'], df['x'])
    r = df['radius'] * 100
    scatter = ax3.scatter(theta, r, c=df['energy_keV'], cmap='plasma', s=5, alpha=0.7)
    ...
    ax5.axvline(x=INNER_GRID_RADIUS_CM, color='red', linestyle='--', linewidth=1.5, alpha=0.7)
    ax5.axvline(x=OUTER_GRID_RADIUS_CM, color='blue', linestyle='--', linewidth=1.5, alpha=0.7)

    ax6 = fig.add_subplot(2, 3, 6)
    particle_counts = df['particle_type'].value_counts()
    colors = [get_particle_style(p)[0] for p in particle_counts.index]
    labels = [get_particle_style(p)[1] for p in particle_counts.index]
    ax6.pie(particle_counts.values, labels=labels, colors=colors, autopct='%1.1f%%', startangle=90)
...
def plot_phase_space(df: pd.DataFrame):
    """
...
def plot_3d_positions(df: pd.DataFrame):
    """
    ...
    for radius, color, alpha in [(INNER_GRID_RADIUS_CM, 'red', 0.1), (OUTER_GRID_RADIUS_CM, 'blue', 0.05)]:
...
def plot_energy_filter(df: pd.DataFrame, min_keV: float = 0.0, max_keV: float = None):
    """
    ...
    r = filtered['radius'] * 100  # 'radius' already computed in load_data
    ...
    ax.axvline(INNER_GRID_RADIUS_CM, color='red', linestyle='--',
               label=f'Inner grid ({INNER_GRID_RADIUS_CM} cm)', alpha=0.7)
    ax.axvline(OUTER_GRID_RADIUS_CM, color='blue', linestyle='--',
               label=f'Outer grid ({OUTER_GRID_RADIUS_CM} cm)', alpha=0.7)
...
def export_statistics(stats: dict, filepath) -> None:
    """
...
def print_statistics(stats: dict) -> None:
    """
    Print statistics to the terminal in the shared banner format used by
    both the interactive menu and --save mode.
    :param stats: The statistics dictionary.
    """
    print('\n' + '=' * 50)
...
class InteractiveExplorer:
    ...
    def run(self):
        ...
        dispatch = {
            '1': self.show_comprehensive,
            '2': self.show_phase_space,
            '3': self.show_3d,
            '4': self.show_energy_filter,
            '5': self.show_statistics,
            '6': self.energy_filter_settings,
            's': self.save_current,
        }
    ...
    def _display_plot(self, fig, plot_name: str):
        """
        Swap in a newly generated figure, closing the previous one.
        :param fig: The new matplotlib figure to display.
        :param plot_name: Name recorded as the current plot (used by save_current).
        """
        if self.fig:
            plt.close(self.fig)
        self.fig = fig
    ...
    def show_comprehensive(self):
        ...
        self._display_plot(plot_comprehensive(self.df), 'comprehensive')
    ...
    def show_phase_space(self):
        ...
        self._display_plot(plot_phase_space(self.df), 'phase_space')
        print('Plot displayed. Use zoom/pan tools to explore.')
        input('Press Enter to continue...')
    ...
    def show_3d(self):
        ...
        self._display_plot(plot_3d_positions(self.df), '3d')
    ...
    def show_energy_filter(self):
        ...
        self._display_plot(plot_energy_filter(self.df, self.energy_min, self.energy_max), 'energy_filter')
        print('Plot displayed. Use zoom/pan tools to explore.')
        input('Press Enter to continue...')
...
PLOT_BUILDERS = {
    'comprehensive': (plot_comprehensive, '_comprehensive.png'),
    'phase': (plot_phase_space, '_phase_space.png'),
    '3d': (plot_3d_positions, '_3d.png'),
    'energy': (plot_energy_filter, '_energy.png'),
}


def run_batch_mode(df: pd.DataFrame, stats: dict, args) -> None:
    """
    Save static plot images instead of launching the interactive explorer.
    :param df: The dataframe with particle data.
    :param stats: Precomputed statistics.
    ...
    if args.all or 'all' in plot_types:
        plot_types = list(PLOT_BUILDERS.keys())

    for plot_type in plot_types:
        builder = PLOT_BUILDERS.get(plot_type)
        if builder is None:
            continue
        plot_fn, suffix = builder
        fig = plot_fn(df)
        output_file = f'{args.output}{suffix}'
...
def main():
    """
    Parse arguments and dispatch to interactive or batch (--save) mode.
    """
